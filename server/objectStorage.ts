...
export class ObjectStorageService {
  ...
  async getLogoUploadURL(): Promise<string> {
    ...
    const logoId = `logo-${Date.now()}.png`;
    const fullPath = `${publicSearchPaths[0]}/${logoId}`;
  ...
  // Normalizes a logo upload URL to a public path
  normalizeLogoPath(uploadUrl: string): string {
    ...
    if (pathParts.length >= 3) {
      // Remove bucket name and return relative path
